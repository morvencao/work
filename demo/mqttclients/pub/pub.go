...
func NewPub() *cobra.Command {
	...
	var resourceID = "b1e0ccaa-1d84-49dc-a98a-31a6fb2062cc"
	...
	flags.StringVar(&resourceID, "resouce-id", resourceID, "The ID of the resource")
...
func Run(broker, username, password string, clusterName, resourceID string, message string) {
	...
	topic := fmt.Sprintf("/v1/%s/%s/content", clusterName, resourceID)
